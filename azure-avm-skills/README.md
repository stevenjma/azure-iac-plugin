...
> **POC status.** This is a proof of concept. The harvest oracle calls the ARM control-plane REST
> API directly (via `az rest`) behind a single `export-dispatch` seam. A remote ARM MCP server, when
> wired, serves the Azure Resource Graph discovery and Bicep what-if operations it exposes. Module
> resolution reads the public AVM index + MCR / Terraform Registry version feeds.
...
**No hard dependency on the remote ARM MCP.** The harvest oracle always calls the ARM control-plane
...
Configuring `arm-mcp` is recommended for its ARG discovery and what-if tools, but the pipeline
never blocks on it.
...
copilot plugin install ./azure-avm-skills
```
