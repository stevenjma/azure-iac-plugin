# Azure-to-Terraform translation rules

Load this reference only after a validation error or plan diff matches a category in the parent
skill's diagnostic index. The rules apply equally to local `aztfexport` output and the server-side
`Microsoft.AzureTerraform/exportTerraform` action because both use the same export engine.

Confirm enum values, defaults, computed/write-only fields, and resource shapes with the matching
provider schema before applying a rule. Prefer Terraform Registry documentation or
`terraform providers schema -json`; use ARM resource-type schemas only to confirm the Azure-side
shape. Never copy a value from a different provider's schema.
